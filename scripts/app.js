...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function actualizarResumenTallas() {
        const tallas = [
            "EXTRA CHICA", "CHICA", "MEDIANA", 
            "GRANDE", "EXTRA GRANDE", "EXTRA EXTRA GRANDE"
        ];
        
        const resumen = tallas.map(talla => {
            return {
                talla,
                hombre: registrosFiltrados.filter(r => 
                    r.get("talla") === talla && r.get("genero") === "Hombre"
                ).length,
                mujer: registrosFiltrados.filter(r => 
                    r.get("talla") === talla && r.get("genero") === "Mujer"
                ).length
            };
        });
    ...
    function generarPDF() {
        ...
        const resumenTallas = tallas.map(talla => {
            const hombre = registrosFiltrados.filter(r => 
                r.get("talla") === talla && r.get("genero") === "Hombre"
            ).length;
            
            const mujer = registrosFiltrados.filter(r => 
                r.get("talla") === talla && r.get("genero") === "Mujer"
            ).length;
            
            return [talla, hombre, mujer, hombre + mujer];
        });
        
        doc.autoTable({
            head: [['Talla', 'Hombre', 'Mujer', 'Total']],
            body: resumenTallas,
            startY: 60,
            styles: {
                fontSize: 9,
        ...
        // Tabla de registros
        const registrosPDF = registrosFiltrados.map(reg => {
            const totalAbonado = reg.get("totalAbonado") || 0;
            const restante = Math.max(0, 200 - totalAbonado);
        ...
        doc.autoTable({
            head: [['Nombre', 'Talla', 'Género', 'Abonado', 'Restante', 'Estado']],
            body: registrosPDF,
